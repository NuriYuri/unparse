class BeginNode < OverridableNode
  ...
  def as_node(type = @type, children = @children, props = @props)
    ...
    cleaned_children = children.reject { |c| c.is_a?(OverridableNode) && c.removed }
    return super(type, cleaned_children, props) if cleaned_children.size > 1 || cleaned_children.size == children.size

    child = cleaned_children[0]
    ...
    return super(type, cleaned_children, props) 
  end
